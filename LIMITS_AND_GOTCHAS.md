...
- **1 GB storage/RAM, 1 shared CPU, 20 max connections.** Fine for hobby-scale shipment volumes; keep an eye on storage if usage grows. (Supabase is an equally good alternative at 500MB — see `POSTGRES_SETUP.md`.)
...
- **`sslmode=require` in Aiven's connection string breaks asyncpg** — must be rewritten to `ssl=true`, or SQLAlchemy throws `TypeError: unexpected keyword argument 'sslmode'` on connect. Already called out in `POSTGRES_SETUP.md`; worth repeating here since it's an easy one to reintroduce if you ever copy-paste a fresh connection string from the Aiven dashboard without editing it.
